...
- SQL Injection vs Input Validation — SQL Injection: Attack that manipulates SQL queries through untrusted input within web, data, and cloud services. Input Validation: Process of checking user input for format, length, and allowed values within web, data, and cloud services.
- Purpose vs mechanism — be able to separate what SQL Injection is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. SQL Injection: works when untrusted input changes query structure, allowing the database to execute unintended commands If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Describe what SQL Injection does, what input it works on, and what evidence would show that it is functioning correctly.
