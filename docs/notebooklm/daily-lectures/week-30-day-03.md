...
- Triage Queue vs Alert Triage — Triage Queue: Ordered list of alerts or cases awaiting analyst review. Alert Triage: Initial analysis process that validates and prioritizes alerts.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Detection Rule: evaluates events against defined conditions, thresholds, or sequences and generates alerts when matching activity appears Triage Queue: ranks incoming work by severity, confidence, and business impact so limited analyst time goes to the riskiest items first If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between Detection Rule and Triage Queue without using the words "better" or "faster" as your whole answer.
