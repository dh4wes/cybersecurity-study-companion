...
- VLAN vs LAN — VLAN: Virtual LAN used to segment broadcast domains on switched networks. LAN: Local area network that connects devices across a limited physical space such as a room, office, or campus.
- Spanning Tree Protocol vs VLAN — Spanning Tree Protocol: Protocol that prevents switching loops by managing redundant paths. VLAN: Virtual LAN used to segment broadcast domains on switched networks.
- Purpose vs mechanism — be able to separate what VLAN is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. VLAN: tags frames with a logical network ID so one switch fabric can carry multiple isolated broadcast domains Spanning Tree Protocol: elects a root bridge and blocks redundant links so layer 2 loops do not form If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between VLAN and Spanning Tree Protocol without using the words "better" or "faster" as your whole answer.
- Describe what VLAN does, what input it works on, and what evidence would show that it is functioning correctly.
