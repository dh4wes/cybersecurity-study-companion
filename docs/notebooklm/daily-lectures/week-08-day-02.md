...
- Purpose vs mechanism — be able to separate what Network ID is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Network ID: applies the subnet mask to an address, keeps the network bits, and treats the result as the subnet identifier Host Range: starts after the network address, ends before the broadcast address, and provides the usable values between those boundaries If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between Network ID and Host Range without using the words "better" or "faster" as your whole answer.
- Describe what Network ID does, what input it works on, and what evidence would show that it is functioning correctly.
