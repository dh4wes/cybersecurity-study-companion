...
- Purpose vs mechanism — be able to separate what PCAP is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. PCAP: stores captured packets with timestamps and headers so tools can replay, filter, or inspect traffic later Packet Filter: inspects header fields such as address, port, and protocol and then applies matching allow or deny rules If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between PCAP and Packet Filter without using the words "better" or "faster" as your whole answer.
- Describe what PCAP does, what input it works on, and what evidence would show that it is functioning correctly.
