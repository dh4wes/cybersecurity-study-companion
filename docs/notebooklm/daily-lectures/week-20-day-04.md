...
- Eradication vs Containment — Eradication: Removal of attacker presence, malicious artifacts, and root causes. Containment: Response action that limits incident spread and immediate damage.
- Purpose vs mechanism — be able to separate what Eradication is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Eradication: removes malicious artifacts, closes exploited weaknesses, and validates cleanup so the same foothold cannot immediately return If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Describe what Eradication does, what input it works on, and what evidence would show that it is functioning correctly.
