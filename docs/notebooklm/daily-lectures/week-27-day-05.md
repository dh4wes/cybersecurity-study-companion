...
- Lateral Movement vs Containment — Lateral Movement: Post-compromise activity where an attacker moves from one system to other reachable systems. Containment: Response action that limits incident spread and immediate damage.
- Purpose vs mechanism — be able to separate what Lateral Movement is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Lateral Movement: uses stolen access, trust paths, or remote tools to pivot from the initial foothold into additional internal systems If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Describe what Lateral Movement does, what input it works on, and what evidence would show that it is functioning correctly.
