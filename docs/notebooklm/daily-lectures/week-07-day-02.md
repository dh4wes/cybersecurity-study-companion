...
- Purpose vs mechanism — be able to separate what Broadcast Domain is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Broadcast Domain: switches flood broadcasts within the same VLAN or segment, but routers and segmentation boundaries prevent the traffic from crossing further Switch: learns MAC-to-port mappings from incoming frames and forwards later traffic only toward the needed port If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between Broadcast Domain and Switch without using the words "better" or "faster" as your whole answer.
- Describe what Broadcast Domain does, what input it works on, and what evidence would show that it is functioning correctly.
