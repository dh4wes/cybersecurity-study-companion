...
- Hardening vs Patch Management — Hardening: Process of reducing attack surface through secure configuration. Patch Management: Process for testing, deploying, and verifying software updates.
- Purpose vs mechanism — be able to separate what Network Baseline is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Network Baseline: learns normal volumes, peers, ports, and timing so unusual deviations can be flagged as suspicious Hardening: reduces attack surface by disabling unnecessary features, enforcing secure settings, and removing weak defaults before exposure If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.

## 6. If you can explain this, you understand it
- Explain the difference between Network Baseline and Hardening without using the words "better" or "faster" as your whole answer.
- Describe what Network Baseline does, what input it works on, and what evidence would show that it is functioning correctly.
