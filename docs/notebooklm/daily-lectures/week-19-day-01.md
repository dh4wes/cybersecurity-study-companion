...
- Purpose vs mechanism — be able to separate what Mitigation is for from how it actually performs that job.
...
Use a pipeline model for this day: input arrives, a component or service processes it, state changes occur, and an output or symptom becomes visible to the user, administrator, or analyst. Mitigation: reduces either the chance or the consequence of a risk through selected safeguards and process changes Exploit: uses crafted input or sequence timing to trigger a weakness and execute behavior the system did not intend to allow If you can narrate that path from start to finish without skipping the middle, the topic is becoming operational instead of abstract.
